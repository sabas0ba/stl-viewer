...
var MESH_FS = [
  ...
  'uniform int uShadeMode;',   // 0: 単色, 1: オーバーハング, 2: 高さ, 3: ライト, 4: AO近似+ライト
  ...
  '  }',
  '  if(!front) base *= 0.45;',
  '  vec3 lightA = normalize(uLightPos - vWorld);',
  ...
  '  float light = amb + dA * 0.62 + dB;',
  '  if(uShadeMode == 3 || uShadeMode == 4){',
  '    float distanceAttenuation = 1.0 / (1.0 + 0.12 * length(uLightPos - vWorld) / max(uSceneRadius, 1e-3));',
  '    light = uAmbientStrength + dA * 0.95 * distanceAttenuation + dB * 0.55;',
  '    light *= uLightStrength;',
  '    light += uAmbientStrength * 0.35;',
  '  }',
  '  if(uShadeMode == 4){',
  '    // 深度テクスチャを使わない、下面・凹部側を強調する AO 近似',
  '    float ambientOcclusion = 1.0 - uAoStrength * (1.0 - smoothstep(-0.35, 0.75, n.z));',
